...
{
    internal static class DualSenseRenderDeviceSelector
    {
        ...
        private static int ComputeScore(string? friendlyName)
        {
            ...
            if (friendlyName.Contains("Haut-parleur", StringComparison.OrdinalIgnoreCase))
                score += 5;

            if (friendlyName.Contains("Speaker", StringComparison.OrdinalIgnoreCase))
                score += 5;
        ...
        private static string BuildDiagnostics(dynamic[] scoredDevices)
        {
            if (scoredDevices.Length == 0)
                return "Aucun périphérique de rendu actif compatible DualSense n'a été trouvé.";

            var sb = new StringBuilder();
            sb.Append("Candidats audio DualSense: ");
